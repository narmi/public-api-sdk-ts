/* tslint:disable */
/* eslint-disable */
/**
 * Narmi Public API
 * To read about Public API access and authentication, go to [API Overview](https://docs.narmi.com/docs/narmi-developer-docs/xl9dvbz84o11l-introduction).
 *
 * The version of the OpenAPI document: v1 (v2026.8.177)
 * Contact: support@narmi.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { StdEntClsCodeEnum } from './StdEntClsCodeEnum';
import {
    StdEntClsCodeEnumFromJSON,
    StdEntClsCodeEnumFromJSONTyped,
    StdEntClsCodeEnumToJSON,
} from './StdEntClsCodeEnum';

/**
 * 
 * @export
 * @interface ScheduledACHPaymentRequest
 */
export interface ScheduledACHPaymentRequest {
    /**
     * This value is expressed as an integer in the currency's smallest unit (e.g. cents), not as a decimal. For example, $5.00 is represented as 500 and $0.99 as 99.
     * @type {number}
     * @memberof ScheduledACHPaymentRequest
     */
    amount: number;
    /**
     * 
     * @type {string}
     * @memberof ScheduledACHPaymentRequest
     */
    deviceId?: string | null;
    /**
     * 
     * @type {string}
     * @memberof ScheduledACHPaymentRequest
     */
    deviceRequestId?: string | null;
    /**
     * 
     * @type {string}
     * @memberof ScheduledACHPaymentRequest
     */
    memo?: string;
    /**
     * 
     * @type {string}
     * @memberof ScheduledACHPaymentRequest
     */
    recurringRule: string;
    /**
     * A field that represents the stringified UUID of a related object.
     * @type {string}
     * @memberof ScheduledACHPaymentRequest
     */
    institutionAccount: string;
    /**
     * A field that represents the stringified UUID of a related object.
     * @type {string}
     * @memberof ScheduledACHPaymentRequest
     */
    recipient: string;
    /**
     * A field that represents the stringified UUID of a related object.
     * @type {string}
     * @memberof ScheduledACHPaymentRequest
     */
    achCompany?: string;
    /**
     * 
     * @type {StdEntClsCodeEnum}
     * @memberof ScheduledACHPaymentRequest
     */
    stdEntClsCode: StdEntClsCodeEnum;
    /**
     * 
     * @type {string}
     * @memberof ScheduledACHPaymentRequest
     */
    entryDesc: string;
    /**
     * 
     * @type {string}
     * @memberof ScheduledACHPaymentRequest
     */
    description?: string;
}

/**
 * Check if a given object implements the ScheduledACHPaymentRequest interface.
 */
export function instanceOfScheduledACHPaymentRequest(value: object): value is ScheduledACHPaymentRequest {
    if (!('amount' in value) || value['amount'] === undefined) return false;
    if (!('recurringRule' in value) || value['recurringRule'] === undefined) return false;
    if (!('institutionAccount' in value) || value['institutionAccount'] === undefined) return false;
    if (!('recipient' in value) || value['recipient'] === undefined) return false;
    if (!('stdEntClsCode' in value) || value['stdEntClsCode'] === undefined) return false;
    if (!('entryDesc' in value) || value['entryDesc'] === undefined) return false;
    return true;
}

export function ScheduledACHPaymentRequestFromJSON(json: any): ScheduledACHPaymentRequest {
    return ScheduledACHPaymentRequestFromJSONTyped(json, false);
}

export function ScheduledACHPaymentRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): ScheduledACHPaymentRequest {
    if (json == null) {
        return json;
    }
    return {
        
        'amount': json['amount'],
        'deviceId': json['device_id'] == null ? undefined : json['device_id'],
        'deviceRequestId': json['device_request_id'] == null ? undefined : json['device_request_id'],
        'memo': json['memo'] == null ? undefined : json['memo'],
        'recurringRule': json['recurring_rule'],
        'institutionAccount': json['institution_account'],
        'recipient': json['recipient'],
        'achCompany': json['ach_company'] == null ? undefined : json['ach_company'],
        'stdEntClsCode': StdEntClsCodeEnumFromJSON(json['std_ent_cls_code']),
        'entryDesc': json['entry_desc'],
        'description': json['description'] == null ? undefined : json['description'],
    };
}

export function ScheduledACHPaymentRequestToJSON(value?: ScheduledACHPaymentRequest | null): any {
    if (value == null) {
        return value;
    }
    return {
        
        'amount': value['amount'],
        'device_id': value['deviceId'],
        'device_request_id': value['deviceRequestId'],
        'memo': value['memo'],
        'recurring_rule': value['recurringRule'],
        'institution_account': value['institutionAccount'],
        'recipient': value['recipient'],
        'ach_company': value['achCompany'],
        'std_ent_cls_code': StdEntClsCodeEnumToJSON(value['stdEntClsCode']),
        'entry_desc': value['entryDesc'],
        'description': value['description'],
    };
}

