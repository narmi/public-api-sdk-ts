...
export class MfaApi extends runtime.BaseAPI {
    ...
    /**
     * Delete a user\'s MFA device.  All monetary values are represented in minor units, or the smallest unit of the currency with no decimal (e.g., cents). For example, $10.00 is represented as 1000.    <i>How can we improve these docs?             <a href=\"mailto:docs+feedback@narmi.com\">Share your feedback.</a></i>
     * Delete MFA device
     */
    ...
    /**
     * List the authenticated user\'s MFA devices.  All monetary values are represented in minor units, or the smallest unit of the currency with no decimal (e.g., cents). For example, $10.00 is represented as 1000.    <i>How can we improve these docs?             <a href=\"mailto:docs+feedback@narmi.com\">Share your feedback.</a></i>
     * List MFA devices
     */
    ...
    /**
     * Elevate user permissions using Two-Factor-Authentication.     Send a `device_id` to generate and dispatch a one-time-password to the specified device.     Send `device_id` and `one_time_password` to validate the OTP sent to a user.     The endpoint will return a `403` if there is an issue with the `device_id` or `one_time_password`, or if the user has no registered 2FA devices.  All monetary values are represented in minor units, or the smallest unit of the currency with no decimal (e.g., cents). For example, $10.00 is represented as 1000.    <i>How can we improve these docs?             <a href=\"mailto:docs+feedback@narmi.com\">Share your feedback.</a></i>
    ...
    /**
     * Retrieve whether the current user holds **sudo** (elevated authentication).  Sudo is a temporary, elevated-authentication state required to perform certain sensitive operations. Whether a given operation requires sudo is configured per institution, and the list below is not exhaustive.  **Operations that may require sudo**  - Send an ACH payment or originate an unverified ACH transaction - Create an external push or instant (FedNow) transfer - Create a wire - Add an external account  **Obtaining sudo**  1. `GET /sudo` returns `200` if you already hold sudo, or `404` if you do not. 2. `POST /sudo` with a `device_id` to send a one-time password to that 2FA device. 3. `POST /sudo` with `device_id` and `one_time_password` to validate the code and elevate.  Sudo expires after an institution-configured period, after which it must be renewed.  All monetary values are represented in minor units, or the smallest unit of the currency with no decimal (e.g., cents). For example, $10.00 is represented as 1000.    <i>How can we improve these docs?             <a href=\"mailto:docs+feedback@narmi.com\">Share your feedback.</a></i>
