...
export class MfaApi extends runtime.BaseAPI {
    ...
    /**
     * Elevate user permissions using Two-Factor-Authentication.     Send a `device_id` to generate and dispatch a one-time-password to the specified device.     Send `device_id` and `one_time_password` to validate the OTP sent to a user.     The endpoint will return a `403` if there is an issue with the `device_id` or `one_time_password`, or if the user has no registered 2FA devices.
    ...
    async sudoCreateRaw(requestParameters: SudoCreateRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<CreateSudo>> {
        if (requestParameters['createSudoRequest'] == null) {
            throw new runtime.RequiredError(
                'createSudoRequest',
                'Required parameter "createSudoRequest" was null or undefined when calling sudoCreate().'
            );
        }
    ...
    /**
     * Retrieve information on whether the current user has sudo permissions.
