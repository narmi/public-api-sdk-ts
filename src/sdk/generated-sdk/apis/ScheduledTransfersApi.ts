...
export class ScheduledTransfersApi extends runtime.BaseAPI {
    ...
    async transfersScheduledCreateRaw(requestParameters: TransfersScheduledCreateRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<NestedScheduledTransfer>> {
        ...
        if (this.configuration && this.configuration.accessToken) {
            // oauth required
            headerParameters["Authorization"] = await this.configuration.accessToken("oauth2", ["banking:scheduled_transfers:write"]);
        }
    ...
    /**
     * Delete a user\'s scheduled transfer. Accepts a payload containing an identifier for the scheduled transfer, either a Narmi UUID or a core identifier.  Example: ```http DELETE /v1/transfers/scheduled/ HTTP/1.1 Host: api.example.com Content-Type: application/json  {   \"id\": \"07231f46-23f1-482e-bdc4-35f2267b60e6\" }  ```  All monetary values are represented in minor units, or the smallest unit of the currency with no decimal (e.g., cents). For example, $10.00 is represented as 1000.    <i>How can we improve these docs?             <a href=\"mailto:docs+feedback@narmi.com\">Share your feedback.</a></i>
    ...
    /**
     * List the authenticated user\'s scheduled transfers.  All monetary values are represented in minor units, or the smallest unit of the currency with no decimal (e.g., cents). For example, $10.00 is represented as 1000.    <i>How can we improve these docs?             <a href=\"mailto:docs+feedback@narmi.com\">Share your feedback.</a></i>
    ...
    async transfersScheduledListRaw(requestParameters: TransfersScheduledListRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<PaginatedScheduledTransferList>> {
        ...
        if (this.configuration && this.configuration.accessToken) {
            // oauth required
            headerParameters["Authorization"] = await this.configuration.accessToken("oauth2", ["banking:scheduled_transfers:read"]);
        }
