...
import type {
  ...
  InvalidRequestError,
  NotFoundError,
  PatchedCheckDetailUpdateRequest,
...
import {
    ...
    CheckDetailUpdateFromJSON,
    CheckDetailUpdateToJSON,
    InvalidRequestErrorFromJSON,
    InvalidRequestErrorToJSON,
    NotFoundErrorFromJSON,
    NotFoundErrorToJSON,
    PatchedCheckDetailUpdateRequestFromJSON,
    PatchedCheckDetailUpdateRequestToJSON,
...
export interface TransactionCheckDetailUpdateRequest {
    checkDetailUuid: string;
    uuid: string;
    patchedCheckDetailUpdateRequest?: PatchedCheckDetailUpdateRequest;
}
...
export class TransactionsApi extends runtime.BaseAPI {
    ...
    /**
     * Update user-override fields (pay_from, pay_to, memo, check_date) on a transaction\'s check detail. Override values take precedence over OCR-extracted values when displayed.  All monetary values are represented in minor units, or the smallest unit of the currency with no decimal (e.g., cents). For example, $10.00 is represented as 1000.    <i>How can we improve these docs?             <a href=\"mailto:docs+feedback@narmi.com\">Share your feedback.</a></i>
     * Update check detail overrides
     */
    async transactionCheckDetailUpdateRaw(requestParameters: TransactionCheckDetailUpdateRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<CheckDetailUpdate>> {
        if (requestParameters['checkDetailUuid'] == null) {
            throw new runtime.RequiredError(
                'checkDetailUuid',
                'Required parameter "checkDetailUuid" was null or undefined when calling transactionCheckDetailUpdate().'
            );
        }
        ...
        if (requestParameters['uuid'] == null) {
            throw new runtime.RequiredError(
                'uuid',
                'Required parameter "uuid" was null or undefined when calling transactionCheckDetailUpdate().'
            );
        }
        ...
        const response = await this.request({
            path: `/v1/transactions/{uuid}/check-detail/{check_detail_uuid}/`.replace(`{${"check_detail_uuid"}}`, encodeURIComponent(String(requestParameters['checkDetailUuid']))).replace(`{${"uuid"}}`, encodeURIComponent(String(requestParameters['uuid']))),
            method: 'PATCH',
            headers: headerParameters,
            query: queryParameters,
            body: PatchedCheckDetailUpdateRequestToJSON(requestParameters['patchedCheckDetailUpdateRequest']),
        }, initOverrides);

        return new runtime.JSONApiResponse(response, (jsonValue) => CheckDetailUpdateFromJSON(jsonValue));
    }
     ...
     * Update check detail overrides
     */
    async transactionCheckDetailUpdate(requestParameters: TransactionCheckDetailUpdateRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<CheckDetailUpdate> {
        const response = await this.transactionCheckDetailUpdateRaw(requestParameters, initOverrides);
    ...
    async transactionsUpdateRaw(requestParameters: TransactionsUpdateRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<TransactionsDetail>> {
        ...
        if (this.configuration && this.configuration.accessToken) {
            // oauth required
            headerParameters["Authorization"] = await this.configuration.accessToken("oauth2", ["banking:transactions:write"]);
        }
